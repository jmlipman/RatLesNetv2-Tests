...
class UNet3D(ModelBase):
    """3D U-Net model.
       ...
       Required memory (nvidia-smi): 0.65, 7252MB, 7621MiB
       Number of parameters: 19M (19073730)

       Configuration directly drawn from the paper:
        * Batch size: 1
       ...
       Modifications:
        * Convolutions are padded, otherwise the slices fade out.
        * No tiles, end-to-end segmentation.
        * They originally have to separate between cells, so they use a
          weighted loss. I use a different weighted loss. If I don't use
          any weighted loss, the model only predicts background.
        * I don't crop any tensor.
        * They add a dropout layer at the end for data augmentation. I don't.
        * I don't use data augmentation. They: scale, rotate, grey, transform.

       Notes:
        * They don't say when to stop training.
        * They don't specify the learning rate.
        * They don't consider shifts in the data.
    """
    ...
    def create_loss(self):
        """Weighted loss. In the original paper they set weight=0 unlabelled
           voxels.
        """
    ...
    def create_model(self):
        ...
        # Bottleneck output
        dec_out4 = UNetBottleneckBlock(self.config, nfi=self.config["classes"])(dec_out3)
